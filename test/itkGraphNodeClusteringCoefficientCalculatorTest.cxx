...
int itkGraphNodeClusteringCoefficientCalculatorTest( int argc, char * argv [] )
{
  ...
  if ( argc >= 2 )
    ...
    matrix.SetSize( reader->GetVariableDimensions(0)->GetElement(0),
                    reader->GetVariableDimensions(0)->GetElement(1) );

    std::cout << "Data file: " << argv[1] << std::endl;
    std::cout << "Data variable: " << reader->GetVariableName(0) << std::endl;
    std::cout << "Matrix size = " << matrix.Rows() << "x" << matrix.Cols() << std::endl;
    
    matrix.Fill(0);
    
    for ( unsigned int i=0; i<reader->GetVariableDimensions(0)->GetElement(0); i++) 
      {
      for ( unsigned int j=0; j<reader->GetVariableDimensions(0)->GetElement(1); j++) 
        {
        matrix(i,j) = reader->GetVariableData(0)->GetElement( i*reader->GetVariableDimensions(0)->GetElement(1) + j );
        }
      }
  ...
  std::cout << "Unweighted clustering coefficients" << std::endl;
  calc->ComputeClusteringCoefficient();
  for (unsigned int i=0; i<graph->GetTotalNumberOfNodes(); i++)
    {
    std::cout << calc->GetClusteringCoefficient( i ) << " ";
    }
  std::cout << std::endl;
  
  std::cout << "Weighted clustering coefficients" << std::endl;
