...
int itkGraphNodeDegreeCentralityCalculatorTest( int argc, char * argv [] )
{
  ...
  for ( unsigned int i=0; i<graph->GetTotalNumberOfEdges(); i++)
    {
    EdgePointerType edge =  graph->GetEdgePointer( i );
    if ( (edge->SourceIdentifier == 5) || (edge->TargetIdentifier == 5 ))
      {
      std::cout << edge->SourceIdentifier << " -> " << edge->TargetIdentifier << std::endl;
      }
    }
  std::cout << std::endl;
