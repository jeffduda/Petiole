...
=========================================================================*/
#ifndef __itkGraph_hxx
#define __itkGraph_hxx
...
bool
Graph<TGraphTraits>
::HasEdge( NodeIdentifierType SourceNode, NodeIdentifierType TargetNode ) const
{
  EdgeIteratorType It;

  for ( It = this->m_Edges->Begin(); It != this->m_Edges->End(); ++It )
    {
    if ( (It.Value().SourceIdentifier == SourceNode ) &&
         (It.Value().TargetIdentifier == TargetNode ) )
      {
      return true;
      }
    }
...
template<typename TGraphTraits>
typename Graph<TGraphTraits>::GraphTraitsType::NodeIdentifierContainerType
Graph<TGraphTraits>
::GetOutgoingNeighbors( NodeIdentifierType Id ) const
{
  EdgeIdentifierContainerType edges = this->GetNode( Id ).OutgoingEdges;
  NodeIdentifierContainerType neighbors( edges.size() );
  
  for (unsigned int i=0; i<edges.size(); i++)
    {
    neighbors[i] = this->GetEdge( edges[i] ).TargetIdentifier;
    }
...
typename Graph<TGraphTraits>::GraphTraitsType::NodeIdentifierContainerType
Graph<TGraphTraits>
::GetIncomingNeighbors( NodeIdentifierType Id ) const
{
  EdgeIdentifierContainerType edges = this->GetNode( Id ).IncomingEdges;
  NodeIdentifierContainerType neighbors( edges.size() );
  
  for (unsigned int i=0; i<edges.size(); i++)
    {
    neighbors[i] = this->GetEdge( edges[i] ).SourceIdentifier;
    }
...
typename Graph<TGraphTraits>::GraphTraitsType::NodeIdentifierContainerType
Graph<TGraphTraits>
::GetAllNeighbors( NodeIdentifierType Id ) const
{
  NodeIdentifierContainerType iNeighbors = this->GetIncomingNeighbors( Id );
  NodeIdentifierContainerType oNeighbors = this->GetOutgoingNeighbors( Id );
  NodeIdentifierContainerType neighbors( iNeighbors.size() + oNeighbors.size() );

  for (unsigned int i=0; i<oNeighbors.size(); i++)
    {
    neighbors[i] = oNeighbors[i];
    }
  for (unsigned int i=0; i<iNeighbors.size(); i++)
    {
    neighbors[i+oNeighbors.size()] = iNeighbors[i];
    }
  
  std::sort( oNeighbors.begin(), oNeighbors.end() );
  std::unique( oNeighbors.begin(), oNeighbors.end() );
...
{
  NodeIteratorType nIt;
  for ( nIt = this->m_Nodes->Begin(); nIt != this->m_Nodes->End(); ++nIt )
    {
    NodePointerType node = &nIt.Value();
  ...
  for ( It = this->m_Edges->Begin(); It != this->m_Edges->End(); ++It )
    {
    EdgePointerType edge = &It.Value();
    this->GetNodePointer( edge->SourceIdentifier )->OutgoingEdges.push_back( edge->Identifier );
    this->GetNodePointer( edge->TargetIdentifier )->IncomingEdges.push_back( edge->Identifier );
    }
}
