...
::Augment( EdgePointerType middle )
{
  ...
  for( node = this->m_Output->GetSourceNodePointer( middle ); true
  ...
  /* the sink tree **/
  for( node = this->m_Output->GetTargetNodePointer( middle ); true
    ; node = this->m_Output->GetTargetNodePointer( edge ) )
    {
    edge = node->Parent;
    if( edge == this->m_TerminalEdge )
  ...
  for( node = this->m_Output->GetNodePointer( middle->SourceIdentifier ); true
