...
::AlphaExpansion()
{
  if( this->GetNumberOfInputs()-1 != this->GetNumberOfClasses() )
    {
    itkExceptionMacro( "Incorrect number of inputs. "
      << "Input should consist of 1 input image and a likelihood image "
      << "for each class." );
    }
  ...
  RealType E = 2.0*this->m_CurrentEnergy;
  ...
  while( this->m_CurrentEnergy < E &&
    ...
    for( unsigned int i = 1; i <= this->GetNumberOfClasses(); i++ )
      ...
      if( E < this->m_CurrentEnergy )
        ...
        this->Relabel();
        itkDebugMacro( "New Energy = " << this->m_CurrentEnergy );
      }
    }
  }
}
...
::CalculateCurrentEnergy()
{
  ...
  for( It.GoToBegin(); !It.IsAtEnd(); ++It )
    ...
    if( this->m_ImageToGraphFunctor->IsPixelANode( idx1 ) )
      ...
      /** data term */
      energy += this->m_ImageToGraphFunctor->GetSinkDataTerm( idx1 );

      typename ShapedNeighborhoodIterator<OutputImageType>::Iterator sit;
      for( sit = It.Begin(); !sit.IsAtEnd(); sit++ )
        {
        unsigned int j = sit.GetNeighborhoodIndex();
        bool IsInBounds;
...
::FindMinimumEnergyLabeling( unsigned int alpha )
{
  ...
  typename IndexContainerType::const_iterator cit;

  for( unsigned int i = 1; i <= this->GetNumberOfClasses(); i++ )
    {
    if( i != alpha && this->m_Indices.size() > i )
      {
      for( cit = this->m_Indices[i].begin();
        cit != this->m_Indices[i].end(); ++cit )
        {
        sink_index.push_back( *cit );
        }
      }
    }
  ...
  this->m_ImageToGraphFunctor->SetSourceLikelihoodImage( source );
  this->m_ImageToGraphFunctor->SetSinkLikelihoodImage( sink );
  bool tmp_ExcludeBackground
    = this->m_ImageToGraphFunctor->GetExcludeBackground();
...
::GeneratePosteriorEnergyImages()
{
  ...
  NeighborhoodIteratorType It( this->m_ImageToGraphFunctor->GetRadius(),
     this->m_LabelImage, this->m_LabelImage->GetBufferedRegion() );
  It.ClearActiveList();
  typename BoykovImageToGraphFunctorType::IndexListType::const_iterator cit;
  for( cit = this->m_ImageToGraphFunctor->GetActiveIndexList().begin();
    cit != this->m_ImageToGraphFunctor->GetActiveIndexList().end(); ++cit )
    {
    It.ActivateOffset( It.GetOffset( *cit ) );
    }
  ...
  for( It.GoToBegin(); !It.IsAtEnd(); ++It )
    ...
    if( this->m_ImageToGraphFunctor->IsPixelANode( It.GetIndex() ) )
      ...
      for( unsigned int i = 1; i <= this->GetNumberOfClasses(); i++)
        ...
        typename ProbabilityImageType::Pointer inputI;
        inputI = const_cast<ProbabilityImageType*>(
          static_cast<const ProbabilityImageType*>(
          this->ProcessObject::GetInput( i ) ) );
        this->m_ImageToGraphFunctor->SetSinkLikelihoodImage( inputI );
        ...
        typename ProbabilityImageType::Pointer outputI;
        outputI = const_cast<ProbabilityImageType*>(
          static_cast<const ProbabilityImageType*>(
          this->ProcessObject::GetOutput( i ) ) );
        outputI->SetPixel( It.GetIndex(),
